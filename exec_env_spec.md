...
必須ではありませんが、2つのインスタンスをクラウド上で物理的に近いところに配置するために以下のプレイスメントグループの作成もおすすめです。
...
1. 上記セキュリティグループと(プレイスメントグループ)、当リポジトリ内の userdata.yml をユーザーデータに指定してWindowsインスタンスを2つ起動する。\
